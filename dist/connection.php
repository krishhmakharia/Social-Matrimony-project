    ...
    <body class="tw-bg-black ">
        <div class="tw-flex tw-flex-col-reverse md:tw-flex-row tw-flex-wrap  ">
            ...
            <div class="tw-flex-1 tw-ml-0 md:tw-ml-64 tw-p-6 tw-mt-10  md:tw-mt-0 tw-text-white">
                <h1 class="ubuntu-bold tw-font-bold tw-text-white tw-flex  tw-gap-4 " > <img src="imgs/friend.svg " class="img-fluid tw-w-11 tw-h-11 " alt=""> Connections</h1>
                <div class=" tw-w-full tw-flex tw-flex-col tw-gap-4 tw-p-2">
                    <?php 
                        $query=mysqli_query($conn,"SELECT * FROM details WHERE email IN (SELECT email from intrested where code='$code' AND intrest=1 ) OR code IN (SELECT code from intrested where email='$email' AND intrest=1)");
                        while($row=mysqli_fetch_array($query)){
                            ...
                            <div id="<?=$user_code?>" class=" tw-relative tw-overflow-hidden tw-z-10  tw-w-full md:tw-w-3/4 tw-border-[1px] tw-border-white tw-border-opacity-35 hover:tw-border-opacity-75 tw-p-4 tw-flex tw-rounded-3xl tw-gap-4 ">
                                ...
                                <div class="tw-flex tw-absolute tw-right-10  tw-mt-6 tw-gap-4">
                                    ...
                                    <div data-id="<?=$user_code?>" class="disconnect md:tw-p-4 tw-p-2 hover:tw-bg-yellow-600 tw-rounded-full hover:tw-border-[1px] hover:tw-bg-opacity-80 hover:tw-border-opacity-60 hover:tw-shadow-sm   hover:tw-border-white"><img src="imgs/unfollow.svg" class="img-fluid tw-w-6 tw-h-6  md:tw-w-8  md:tw-h-8" alt=""></div>
                                    <div data-id="<?=$user_code?>" class="blocked md:tw-p-4 tw-p-2 hover:tw-bg-red-600 tw-rounded-full hover:tw-border-[1px] hover:tw-bg-opacity-80 hover:tw-border-opacity-60 hover:tw-shadow-sm   hover:tw-border-white"><img src="imgs/block1.svg" class="img-fluid tw-w-6 tw-h-6 md:tw-w-8  md:tw-h-8" alt=""></div>
                                </div>
                            </div>
                            <?php
                        }

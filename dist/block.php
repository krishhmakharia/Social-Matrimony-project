        ...
        <script>
            $(document).on("click",".unblock",function(){
                ...
                $.post(
                    ...
                    function(data){
                        if(data=="success"){
                            alert("Unblock successfull");
                            $("#"+code).fadeOut(1000);
                        }
                    }     
                );
               // window.location.href = "unblock.php?id="+code;
            });
    ...
    <body class="tw-bg-black ">
        <div class="tw-flex tw-flex-col-reverse md:tw-flex-row tw-flex-wrap  ">
            ...
            <div class="tw-flex-1 tw-ml-0 md:tw-ml-64 tw-p-6 tw-mt-10  md:tw-mt-0 tw-text-white">
                <h1 class="ubuntu-bold tw-font-bold tw-text-white tw-flex  tw-gap-4 " > <img src="imgs/block.svg" class="img-fluid tw-w-11 tw-h-11 " alt=""> Blocked User List</h1>
                <div class=" tw-w-full tw-flex tw-flex-col tw-gap-4 tw-p-2">
                    <?php 
                        $query=mysqli_query($conn,"SELECT * FROM details WHERE email IN (SELECT email from intrested where code='$code' AND intrest=2 AND blockBy='$email' )");
                        while($row=mysqli_fetch_array($query)){
                            ...
                            <div id="<?=$user_code?>" class=" tw-relative tw-overflow-hidden tw-z-10  tw-w-full md:tw-w-3/4 tw-border-[1px] tw-border-white tw-border-opacity-35 hover:tw-border-opacity-75 tw-p-4 tw-flex tw-rounded-3xl tw-gap-4 ">
                                ...
                                <div class="tw-flex tw-absolute tw-right-10  tw-mt-6 tw-gap-4">
                                    ...
                                    <div data-id="<?=$user_code?>" class="unblock md:tw-p-3 tw-p-2 hover:tw-cursor-pointer hover:tw-bg-green-600 tw-rounded-2xl  hover:tw-border-[1px] hover:tw-bg-opacity-80 hover:tw-border-opacity-60 hover:tw-shadow-sm   hover:tw-border-white tw-text-xl tw-font-semibold ubuntu-light ">Unblock</div>
                                </div>
                            </div>
                            <?php
                        }
